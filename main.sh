...
menuPrompt() {
    while true; do
        ...
        printf "\n"
        printf "${tty_bold}${tty_blue} A) Install and run all tasks\n"
        printf "${tty_bold}${tty_blue} D) Clone dot files from GitHub\n"
        printf "${tty_bold}${tty_blue} L) Install packages necessary for Laravel development\n"
        printf "${tty_bold}${tty_blue} S) Download SSH and GPG keys from BitWarden\n"
